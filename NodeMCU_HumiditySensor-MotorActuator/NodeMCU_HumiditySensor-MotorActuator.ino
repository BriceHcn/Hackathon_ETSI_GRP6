...
#ifdef DEBUG
  const long interval = 10000; //temps entre chaque envoie de données au serveur (en ms)
  const long wateringTime = 5000; //temps d'arrosage de la plante
#else
  const long interval = heureInterval*60*60*1000; //temps entre chaque envoie de données au serveur (en ms)
  const long wateringTime = 30000; //temps d'arrosage de la plante
#endif
long currentMillis;
...
void init_humidity() {
  String initialDescription = "Name = HumiditySensor Unit = aVoir Location = Plante";
  String initialData = "0";
  originator = "Cae-HumiditySensor";
  registerModule("HumiditySensor", false, initialDescription, initialData);
}
void task_humidity() {
  String ciContent = "0";
